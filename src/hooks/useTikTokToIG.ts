...
const STORAGE_KEY = 'crown_last_tiktok_search';
...
export interface TikTokToIGResult {
  influencers: Influencer[];
  allProfiled: Influencer[];
  stats: SearchStats;
}

async function runTikTokToIG({ hashtags }: { hashtags: string[] }): Promise<TikTokToIGResult> {
  const res = await fetch('/api/tiktok-to-ig', {
    method: 'POST',
  ...
  const data: TikTokToIGResponse = await res.json();
...
export function useTikTokToIG() {
  const [cached, setCached] = useState<TikTokToIGResult | null>(null);
